 ...
 * </p>
 * <p>
 *     Checks and provides a warning if a class has a Key field
 * </p>
 *
 * @since 1.0
 */
@SupportedAnnotationTypes({"com.roundfeather.persistence.utils.datastore.annotation.DatastoreEntity"})
...
public class DatastoreEntityProcessor extends AbstractProcessor {
     ...
     * and provides a compile time warning if it doesn't that a Long type key is assumed
     *
        ...
        classes.forEach(
                e -> {
                    ...
                    if (!classesWithKey.contains(classElement.getQualifiedName().toString())) {
                        processingEnv.getMessager().printMessage(Diagnostic.Kind.WARNING, String.format("Class '%s' missing DatastoreKey, assuming Long key", classElement.getQualifiedName().toString()), e);
                    }
                }
        );
    }
                ...
                e -> {
                    ...
                    if (deAnnotation.autoGenerateRepository()) {
                        ...
                        try {
                            ...
                            writer.println("import com.roundfeather.persistence.utils.datastore.DatastoreRepository;");
                            writer.println();
